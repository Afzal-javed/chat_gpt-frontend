...
const Summary = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const { data } = await axios.post('https://chat-gpt-backend-jypf.onrender.com/summary', { text });
    ...
    return (
        <Box width={isNotMobile ? '40%' : '80%'} p={'2rem'} m={'2rem auto'}
            ...
            <form onSubmit={handleSubmit}>
                <Typography variant='h4' sx={{ color: 'darkslategray' }} >Summarize</Typography>
            ...
            {summary ? (
                ...
                <Card sx={{ mt: 4, border: 1, boxShadow: 0, height: "500px", borderRadius: 5, borderColor: 'natural.medium', bgcolor: 'background.default' }}>
                    <Typography variant='h6' color='natural.main' sx={{ textAlign: 'center', verticalAlign: 'middel', lineHeight: '450px' }}>Summary Will Appear Here</Typography>
                </Card>
            )}
        </Box>
    )
